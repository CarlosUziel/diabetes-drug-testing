...
class ProjectTesting(unittest.TestCase):
    ...
    def test_create_tf_categorical_feature_cols(self):
        ...
        diabetes_train_ds = df_to_dataset(
            df[student_categorical_col_list + [PREDICTOR]],
            PREDICTOR,
    ...
    def test_create_tf_numerical_feature_cols(self):
        ...
        PREDICTOR = "time_in_hospital"
        diabetes_train_ds = df_to_dataset(
            df[student_numerical_col_list + [PREDICTOR]],
            PREDICTOR,
            batch_size=batch_size,
        )
        diabetes_batch = next(iter(diabetes_train_ds))[0]
        tf_cont_col_list = create_tf_numerical_feature_cols(
            student_numerical_col_list, df[student_numerical_col_list]
        )
